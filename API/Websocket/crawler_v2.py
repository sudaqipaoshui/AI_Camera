...
def _check_display_available():
    """检测当前环境是否可显示 OpenCV 窗口。无 DISPLAY（SSH/headless）时 imshow 会失败或卡住。"""
    if sys.platform == 'win32':
        return True
    return bool(os.environ.get('DISPLAY', '').strip())


# 启动时检测：无图形界面则不调用 imshow，避免报错或卡死；视频仍会写入文件
SHOW_WINDOW = _check_display_available()
...
def check_message_data(pbm, message_size=0):
    ...
    if not pbm.HasField('smart_msg_'):
        ...
        # 检查时间戳
        current_timestamp = pbm.smart_msg_.timestamp_
        if current_timestamp == 0:
            ...
            # 检查帧间隔（延迟）
            time_diff = (current_timestamp - performance_stats['last_timestamp']) / 1000.0  # 转换为秒
                ...
                if len(target.boxes_) == 0:
                    ...
                    for j, box in enumerate(target.boxes_):
                        ...
                        if box.top_left_.x_ >= box.bottom_right_.x_ or box.top_left_.y_ >= box.bottom_right_.y_:
                            errors.append(f"目标 {i} 检测框 {j} 坐标无效: ({box.top_left_.x_}, {box.top_left_.y_}) -> ({box.bottom_right_.x_}, {box.bottom_right_.y_})")
...
def print_check_summary():
    ...
    if len(temperature_stats['temperatures']) > 0:
        ...
        logger.info(f"温度统计: 平均={avg_temp:.1f}°C, 最小={min_temp:.1f}°C, 最大={max_temp:.1f}°C, 采样数={len(temperature_stats['temperatures'])}")
...
def generate_html_report():
    ...
    report_data['summary'] = {
        ...
        'valid_rate': (check_stats['valid_frames'] / check_stats['total_frames'] * 100) if check_stats['total_frames'] > 0 else 0,
...
def generate_json_report():
    ...
    json_report = {
        ...
        'performance': {
            'avg_fps': sum(performance_stats['fps_history']) / len(performance_stats['fps_history']) if len(performance_stats['fps_history']) > 0 else 0,
            ...
            'avg_message_size_kb': sum(performance_stats['frame_sizes']) / len(performance_stats['frame_sizes']) / 1024 if len(performance_stats['frame_sizes']) > 0 else 0,
            'max_message_size_kb': max(performance_stats['frame_sizes']) / 1024 if len(performance_stats['frame_sizes']) > 0 else 0,
            'unique_track_ids': len(performance_stats['track_ids'])
        },
        'temperature': {
            'avg_temp': sum(temperature_stats['temperatures']) / len(temperature_stats['temperatures']) if len(temperature_stats['temperatures']) > 0 else 0,
...
def on_message(ws, message):
    ...
    try:
        ...
        is_Testing = False
        img_w = pbm.img_.width_
        img_h = pbm.img_.height_
        
        # 解码图像
        img = cv2.imdecode(np.frombuffer(pbm.img_.buf_, dtype=np.uint8), 1)
        if img is None:
            print("⚠️  图像解码失败")
            check_stats['errors']['image_decode_failed'] += 1
            return
        ...
        for attr in pbm.Statistics_msg_.attributes_:
            ...
            if attr.type_ == 'areas':
                data = json.loads(attr.value_string_)
            ...
            if attr.type_ == 'sportInfos':
                ...
                for i in range(len(data)):
                    ...
                    tester_data[key]['name'] = data[i]['name']
                    tester_data[key]['testId'] = data[i]['testId']
                    if data[i]['testId'] != '':
                        is_Testing = True
            ...
            if target.type_ == 'person':
                for box in target.boxes_:
                    if box.type_ == 'body':
                        ...
                        print(f'pos_index: {pos_index}, box: ({box.top_left_.x_}, {box.top_left_.y_}) -> ({box.bottom_right_.x_}, {box.bottom_right_.y_})')
                        #pos_index=pid
                        if pos_index > -1:
        ...
        for key in tester_data:
            if key not in video_pose_data:
                video_pose_data[key] = {'LShoulder':[], 'LAnk':[], 'LHand':[], 'RShoulder':[], 'RAnk':[], 'RHand':[]}

        # 处理数据保存
        if is_Testing or True:  # 始终保存（可根据需要修改条件）
            frameid += 1
            print(f"Processing frame {frameid}")
            d = datetime.datetime.fromtimestamp(pbm.smart_msg_.timestamp_ / 1000)
            date_ = d.strftime("%Y%m%d")
            time_ = d.strftime("%H_%M_%S_%f")[:-3]
            
            date_dir = os.path.join(data_path, date_)
            if not os.path.exists(date_dir):
                os.mkdir(date_dir)
            
            if save_type == 'image':
                # 保存图像模式
                json_file = os.path.join(date_dir, time_ + '.json')
                img_file = os.path.join(date_dir, time_ + '.jpg')
                with open(json_file, 'w', encoding='utf-8') as f:
                    json.dump(tester_data, f, ensure_ascii=False, indent=2)
                with open(img_file, 'wb') as f:
                    f.write(pbm.img_.buf_)
            else:
                # 保存视频模式
                ...
                # 显示图像（有图形界面时；无 DISPLAY/headless 时跳过，视频仍会保存）
                if SHOW_WINDOW:
                    try:
                        cv2.imshow('AI Camera', img)
                        cv2.waitKey(1)
                    except Exception as e:
                        pass  # 避免 imshow 异常打断保存
...
if SHOW_WINDOW:
    print("🖼️  画面显示: 开启 (本地窗口)")
else:
    print("🖼️  画面显示: 关闭 (无图形界面，视频仍会保存到文件)")
    if sys.platform != 'win32':
        print("   提示: 需要画面时请设置 DISPLAY 或在有桌面的环境运行")
print("="*60)
...
# 连接超时：避免设备不可达时无限卡住（默认 socket 无超时）
_CONNECT_TIMEOUT = 15  # 秒
_prev_timeout = getattr(__import__('socket'), 'defaulttimeout', None)
try:
    __import__('socket').setdefaulttimeout(_CONNECT_TIMEOUT)
except Exception:
    pass
...
print(f"正在连接 ws://{ip}:8080（超时 {_CONNECT_TIMEOUT}s）...")
try:
    ...
    if _prev_timeout is not None:
        try:
            __import__('socket').setdefaulttimeout(_prev_timeout)
        except Exception:
            pass
