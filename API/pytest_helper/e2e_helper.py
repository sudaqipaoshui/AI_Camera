    ...
    def get_ssh_logs(env: dict, project_root: Path, remote_host: Optional[str] = None,
        ...
        with allure.step(f"获取SSH日志: {remote_host}:{remote_log_path}"):
            ...
            try:
                ...
                proc_result = subprocess.run(scp_cmd, capture_output=True, text=True, timeout=300)
                if proc_result.returncode == 0:
                ...
                result['message'] = "❌ scp 命令执行超时（超过5分钟）"
                print(result['message'])
...
class E2ELogAnalyzer:
    ...
    def analyze_app_log(project_root: Path, log_file: Optional[Path] = None, 
        ...
        # 优先使用 fallback 时间（如果提供）
        if fallback_start_time and fallback_end_time:
            start_time = fallback_start_time
            end_time = fallback_end_time
            print(f"✅ 使用备用时间（fallback）:")
