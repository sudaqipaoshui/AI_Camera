...
@allure.feature('camera-syncCameraSysTime')
@allure.story('syncCameraSysTime')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSyncCameraSysTime(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_syncCameraSysTime.yaml')
    def test_syncCameraSysTime(self, env, inputs, requests, expectation, case, mysql):
