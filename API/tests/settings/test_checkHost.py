...
@allure.feature('camera-checkHost')
@allure.story('checkHost')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestcheckHost(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_checkHost.yaml')
    def test_checkHost(self, env, inputs, requests, expectation, case, mysql):
