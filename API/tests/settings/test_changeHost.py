...
@allure.feature('camera-changeHost')
@allure.story('changeHost')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestAwardCheckin(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_changeHost.yaml')
    def test_award_checkin(self, env, inputs, requests, expectation, case, mysql):
