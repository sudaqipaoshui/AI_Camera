...
@allure.feature('camera-test_getCmaSysInfo')
@allure.story('test_getCmaSysInfo')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestgetCmaSysInfo(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_getCmaSysInfo.yaml')
    def test_getCmaSysInfo(self, env, inputs, requests, expectation, case, mysql):
