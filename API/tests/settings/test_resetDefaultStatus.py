...
@allure.feature('camera-test_resetDefaultStatus')
@allure.story('test_resetDefaultStatus')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestResetDefaultStatus(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_resetDefaultStatus.yaml')
    def test_resetDefaultStatus(self, env, inputs, requests, expectation, case, mysql):
