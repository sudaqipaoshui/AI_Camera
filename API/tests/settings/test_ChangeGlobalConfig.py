...
@allure.feature('camera-ChangeGlobalConfig')
@allure.story('ChangeGlobalConfig')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeGlobalConfig(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_ChangeGlobalConfig.yaml')
    def test_ChangeGlobalConfig(self, env, inputs, requests, expectation, case, mysql):
