...
@allure.feature('camera-test_deleteTester')
@allure.story('test_deleteTester')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestDeleteTester(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_deleteTester.yaml')
    def test_deleteTester(self, env, inputs, requests, expectation, case, mysql):
