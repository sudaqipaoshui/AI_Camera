...
@allure.feature('camera-getCurrentTime')
@allure.story('getCurrentTime')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestGetCurrentTime(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/settings/test_getCurrentTime.yaml')
    def test_getCurrentTime(self, env, inputs, requests, expectation, case, mysql):
