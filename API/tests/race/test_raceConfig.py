...
@allure.feature('camera-test_raceConfig')
@allure.story('raceConfig')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestRaceConfig(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/race/test_raceConfig.yaml')
    def test_raceConfig(self, env, inputs, requests, expectation, case, mysql):
