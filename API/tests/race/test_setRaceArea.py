...
@allure.feature('camera-test_setRaceArea')
@allure.story('test_setRaceArea')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSetRaceAreag(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/race/test_setRaceArea.yaml')
    def test_setRaceArea(self, env, inputs, requests, expectation, case, mysql):
