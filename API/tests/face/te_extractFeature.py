...
@allure.feature("extractFeature")
@allure.story("extractFeature")
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestExtratFeature(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile("API/data/face/test_extractFeature.yaml")
    def test_extractFeature(self, env, inputs, requests, expectation, case, mysql):
