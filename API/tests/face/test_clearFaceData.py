...
@allure.feature('camera-test_clearFaceData')
@allure.story('test_clearFaceData')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestClearFaceData(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/face/test_clearFaceData.yaml')
    def test_clearFaceData(self, env, inputs, requests, expectation, case, mysql):
