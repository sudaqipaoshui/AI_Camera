...
@allure.feature('camera-test_getFaceSyncInfo')
@allure.story('test_getFaceSyncInfo')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestGetFaceSyncInfo(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/face/test_getFaceSyncInfo.yaml')
    def test_getFaceSyncInfo(self, env, inputs, requests, expectation, case, mysql):
