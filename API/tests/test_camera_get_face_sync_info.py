...
@allure.feature('test_camera_get_face_sync_info')
@allure.story('test_camera_get_face_sync_info')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSyncFaceInfo(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/test_camera_get_face_sync_info.yaml')
    def test_camera_get_face_sync_info(self, env, inputs, requests, expectation, case, mysql):
