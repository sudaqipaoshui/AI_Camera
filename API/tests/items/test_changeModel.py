...
@allure.feature('camera-test_changeModel')
@allure.story('test_changeModel')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeModel(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_changeModel.yaml')
    def test_changeModel(self, env, inputs, requests, expectation, case, mysql):
