...
@allure.feature('camera-test_setArea')
@allure.story('test_setArea')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSetArea(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_setArea.yaml')
    def test_setArea(self, env, inputs, requests, expectation, case, mysql):
