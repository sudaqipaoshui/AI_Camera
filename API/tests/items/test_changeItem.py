...
@allure.feature('camera-changeItem')
@allure.story('changeItem')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeItem(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_changeItem.yaml')
    def test_changeItem(self, env, inputs, requests, expectation, case, mysql):
