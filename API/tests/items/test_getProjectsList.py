...
@allure.feature('camera-test_getProjectsList')
@allure.story('test_getProjectsList')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestGetProjectsList(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_getProjectsList.yaml')
    def test_getProjectsList(self, env, inputs, requests, expectation, case, mysql):
