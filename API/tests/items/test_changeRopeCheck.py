...
@allure.feature('camera-test_changeRopeCheck')
@allure.story('test_changeRopeCheck')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeRopeCheck(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_changeRopeCheck.yaml')
    def test_changeRopeCheck(self, env, inputs, requests, expectation, case, mysql):
