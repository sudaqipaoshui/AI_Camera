...
@allure.feature('camera-test_setRole')
@allure.story('test_setRole')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSetRole(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_setRole.yaml')
    def test_setRole(self, env, inputs, requests, expectation, case, mysql):
