...
@allure.feature('camera-test_changeNoTwo')
@allure.story('test_changeNoTwo')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeNoTwo(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_changeNoTwo.yaml')
    def test_changeNoTwo(self, env, inputs, requests, expectation, case, mysql):
