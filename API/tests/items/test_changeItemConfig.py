...
@allure.feature('camera-changeItemConfig')
@allure.story('changeItemConfig')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestChangeItemConfig(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_changeItemConfig.yaml')
    def test_changeItemConfig(self, env, inputs, requests, expectation, case, mysql):
