...
@allure.feature('camera-test_faceMode')
@allure.story('test_faceMode')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestFaceMode(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/items/test_faceMode.yaml')
    def test_faceMode(self, env, inputs, requests, expectation, case, mysql):
