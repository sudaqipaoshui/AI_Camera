...
@allure.feature('camera-uploadVideo')
@allure.story('uploadVideo')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestUploadVideo(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_uploadVideo.yaml')
    def test_uploadVideo(self, env, inputs, requests, expectation, case, mysql):
