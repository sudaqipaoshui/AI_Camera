...
@allure.feature('camera-test_setCameraParam')
@allure.story('test_setCameraParam')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSetCameraParam(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_setCameraParam.yaml')
    def test_setCameraParam(self, env, inputs, requests, expectation, case, mysql):
