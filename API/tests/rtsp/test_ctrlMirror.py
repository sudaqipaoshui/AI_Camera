...
@allure.feature('camera-test_ctrlMirror')
@allure.story('test_ctrlMirror')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestCtrlMirror(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_ctrlMirror.yaml')
    def test_ctrlMirror(self, env, inputs, requests, expectation, case, mysql):
