...
@allure.feature('camera-test_zoomInImage')
@allure.story('test_zoomInImage')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestZoomInImage(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_zoomInImage.yaml')
    def test_zoomInImage(self, env, inputs, requests, expectation, case, mysql):
