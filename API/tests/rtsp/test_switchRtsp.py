...
@allure.feature('camera-test_switchRtsp')
@allure.story('switchRtsp')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestSwitchRtsp(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_switchRtsp.yaml')
    def test_switchRtsp(self, env, inputs, requests, expectation, case, mysql):
