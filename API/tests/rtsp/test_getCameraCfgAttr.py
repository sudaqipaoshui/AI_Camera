...
@allure.feature('camera-test_getCameraCfgAttr')
@allure.story('test_getCameraCfgAttr')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestGetCameraCfgAtt(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_getCameraCfgAttr.yaml')
    def test_getCameraCfgAttr(self, env, inputs, requests, expectation, case, mysql):
