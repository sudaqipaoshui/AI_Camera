...
@allure.feature('camera-getRtspSettings')
@allure.story('getRtspSettings')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestGetRtspSettings(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_getRtspSettings.yaml')
    def test_getRtspSettings(self, env, inputs, requests, expectation, case, mysql):
