...
@allure.feature('camera-test_environmentMode')
@allure.story('test_environmentMode')
# @pytest.mark.flaky(reruns=2, reruns_delay=5)
class TestEnvironmentMode(object):
    @pytest.fixture(scope="class", autouse=True)
    def prepare(self, request, env, mysql, requests):
        pass

    @allure.title("{case}")
    @pytest.mark.datafile('API/data/rtsp/test_environmentMode.yaml')
    def test_environmentMode(self, env, inputs, requests, expectation, case, mysql):
