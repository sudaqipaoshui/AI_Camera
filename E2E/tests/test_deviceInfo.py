...
@allure.feature('camera-deviceInfo')
@allure.story('deviceInfo')
...
class TestDeviceInfo(object):
    ...
    @allure.title("{case}")
    @pytest.mark.datafile('E2E/data/test_deviceInfo.yaml')
    def test_deviceInfon(self, env, inputs, requests, expectation, case, mysql):

        with allure.step(case):
            inputs['host'] = 'http://192.168.3.29'
