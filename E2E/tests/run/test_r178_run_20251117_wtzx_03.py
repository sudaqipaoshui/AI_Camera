...
class TestUpdateRtspSettings(object):
    ...
    # class 作用域的可变容器，用于把 test_updateRtspSettings 计算的时间传给 test_analyze_log（避免类属性在 parametrize 下未传递）
    @pytest.fixture(scope="class")
    def run_log_times(self):
        return {"logStartTime": None, "logEndTime": None}
    ...
    @pytest.mark.datafile('E2E/data/test_updateRtspSettings_178_channel1-4.yaml')
    def test_updateRtspSettings(self, env, inputs, requests, expectation, case, e2e_time_helper, run_log_times):
        """配置RTSP设置并计算时间间隔"""
        with allure.step("计算时间间隔"):
            ...
            # 同时写入 class 作用域 fixture，供 test_analyze_log 使用
            run_log_times["logStartTime"] = time_info['logStartTime']
            run_log_times["logEndTime"] = time_info['logEndTime']
    ...
    def test_ssh_reboot(self, env, e2e_device_manager):
        """使用插件重启设备"""
        devices = ['192.168.3.175', '192.168.3.29']
    ...
    def test_analyze_log(self, e2e_log_analyzer, project_root, run_log_times):
        """统一分析APP和Camera日志 - 使用插件。优先使用 run_log_times（由 test_updateRtspSettings 写入），其次类属性，再 fallback。"""
        app_start = run_log_times.get("logStartTime") or TestUpdateRtspSettings.logStartTime
        app_end = run_log_times.get("logEndTime") or TestUpdateRtspSettings.logEndTime
