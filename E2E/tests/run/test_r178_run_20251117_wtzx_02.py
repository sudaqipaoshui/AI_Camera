...
startTime = '20251117t141019z'
startsportTime = '20251117t141500z'
endTime = '20251117t141959z'
...
class TestUpdateRtspSettings(object):
    ...
    def test_analyze_log(self, e2e_log_analyzer, project_root):
        """统一分析APP和Camera日志 - 使用插件。优先使用 logStartTime/logEndTime，未设置时才用 fallback。"""
        result = e2e_log_analyzer.analyze_all_logs(
            project_root=project_root,
            app_start_time=TestUpdateRtspSettings.logStartTime,  # 优先使用（由 test_updateRtspSettings 设置）
            app_end_time=TestUpdateRtspSettings.logEndTime,      # 优先使用
            expected_circles=r,
            distance=d,
            circles=r
        )
