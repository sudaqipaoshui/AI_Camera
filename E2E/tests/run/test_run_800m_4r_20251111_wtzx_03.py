...
startTime = '20251111t142200z'
startsportTime = '20251111t142800z'
    ...
    def test_analyze_cameralog(self):
        with allure.step("准备分析camera日志所需参数"):
            ...
            if start_time is None or end_time is None:
                ...
                start_time = '2025-12-04 13:43:29'
                end_time = '2025-12-04 13:50:52'
            # 动态获取项目根目录
            project_root = Path(__file__).resolve().parents[3]
            
            # 分析脚本路径
            analyze_script = project_root / "Log" / "analyze_cameralog.py"
            if not analyze_script.exists():
                raise FileNotFoundError(f"分析脚本不存在: {analyze_script}")
            ...
            # 检查日志目录是否存在
            if not app_log_dir.exists() or not app_log_dir.is_dir():
                raise FileNotFoundError(f"APP日志目录不存在: {app_log_dir}")
            if not camera_log_dir.exists() or not camera_log_dir.is_dir():
                raise FileNotFoundError(f"相机日志目录不存在: {camera_log_dir}")
            
            # 查找最新的APP日志和相机日志
            def get_latest_log_file(dir_path):
                """获取目录下最新的.log文件"""
            ...
            app_log_file = get_latest_log_file(app_log_dir)
            camera_log_file = get_latest_log_file(camera_log_dir)
            
            if not app_log_file:
                raise FileNotFoundError(f"在 {app_log_dir} 目录下未找到APP日志文件")
        ...
        with allure.step("执行相机日志分析脚本"):
            try:
                ...
                if not output_dir_abs.exists():
                    raise FileNotFoundError(f"相机日志分析输出目录不存在: {output_dir_abs}")
                
                # 附加分析结果到报告
                output_files = list(output_dir_abs.glob("*"))
                        ...
                        try:
                            with open(file, 'rb') as f:
                                allure.attach(
                                    f.read(),
                                    file.name,
                                    allure.attachment_type.PNG if file.suffix == '.png' else allure.attachment_type.TEXT
                                )
                        except Exception as e:
                            print(f"附加文件 {file.name} 到报告失败: {e}")
