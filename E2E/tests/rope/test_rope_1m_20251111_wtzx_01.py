...
@allure.feature('长跑全流程自动化')
@allure.story('长跑全流程自动化')
   ...
   # 日志开始时间
    ...
    def test_updateRtspSettings(self, env, inputs, requests, expectation, case, mysql):
        ...
        with allure.step("计算时间间隔"):
            ...
            TestUpdateRtspSettings.t2 = t2  # 保存 t2 到类变量，用于后续等待
            
            # 重启到发令的时间间隔 = t0 + t1｜ 3秒倒计时
            time_interval = t0 + t1 - 3
    ...
    def test_click_start_button(self):
        ...
        with allure.step("连接设备并操作"):
            try:
                ...
                # 创建 allure-report 目录和当前日期子目录
                date_str = datetime.datetime.now().strftime("%Y%m%d")
                screenshot_dir = project_root / "allure-report" / "screenshot" / date_str
                screenshot_dir.mkdir(parents=True, exist_ok=True)
                # 生成截图文件名
                screenshot_filename = f"rope_1m_20251111_wtzx_01_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.png"
    ...
    def test_get_log(self):
        with allure.step("等待时间间隔结束"):
            ...
            if TestUpdateRtspSettings.time_interval_2 is None:
                # TestUpdateRtspSettings.time_interval_2 = 6
