...
# 从配置文件读取IP地址
config_file = Path(__file__).resolve().parents[1] / "config" / "test" / "camera.yaml"
ip = "192.168.1.245"  # 默认值
if config_file.exists():
    try:
        with open(config_file, 'r', encoding='utf-8') as f:
            config = yaml.safe_load(f)
            ip = config.get('ssh_host', ip)
            print(f"✅ 从配置文件加载 IP: {ip}")
    except Exception as e:
        print(f"⚠️  读取配置文件失败: {e}，使用默认IP: {ip}")
