...
def download_target_images():
    ...
    pattern = r'save frame/userdata/image/((\d+)_(\d+)_last_(?:right|left)\.jpg)'
    match_result = re.findall(pattern, log_content)

    if not match_result:
        print("日志中未匹配到符合规则的图片链接")
        return

    print(f"第一步-筛选前：共匹配到 {len(match_result)} 张原始图片链接")

    # 筛选逻辑：第二个数值相等+第一个数值差值在100内 → 只保留最后一个
    img_group_dict = {}
    for img_fullname, num1_str, num2_str in match_result:
        num1 = int(num1_str)
        num2 = int(num2_str)
        if num2 not in img_group_dict:
            img_group_dict[num2] = []
        img_group_dict[num2].append((num1, img_fullname))
    ...
    final_download_list = [img_name for (num1, img_name) in final_img_list]
    print(f"第一步-筛选后：共保留 {len(final_download_list)} 张图片待下载 ↓↓↓")

    for num1, img_fullname in final_img_list:
        frame_id = num1
