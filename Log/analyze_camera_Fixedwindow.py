            ...
            for line_num, line in enumerate(f, 1):  # 记录行号，方便排查问题
                ...
                if tester_match:
                    ...
                    if name not in name_set:
                        ...
                        print(f"从第{line_num}行首次提取到name: {name}")
                    else:
                ...
                if not start_time and re.search(start_pattern, line):
                    try:
                        ...
                        print(f"提取到基准时间: {base_time.strftime('%Y-%m-%d %H:%M:%S')}")
                    except:
                        print("警告：未能解析基准时间")
...
def split_by_time_window(records, time_threshold):
    ...
    # 初始化第一个窗口：以第一条记录为窗口起点，初始最后一条为自身
    window_start = sorted_records[0]['timestamp']
    current_last = sorted_records[0]
    
    # 从第二条记录开始遍历
    for record in sorted_records[1:]:
        # 计算与窗口起点的时间差（秒）
        time_diff = (record['timestamp'] - window_start).total_seconds()
...
def plot_db_time_diff_chart(db_data, base_time, output_dir, person_info):
    ...
    all_ids = sorted(db_data.keys(), key=lambda x: int(x.replace('未知-', '')) if x.replace('未知-', '').isdigit() else float('inf'))
    ...
    # 调整布局并保存
    plt.tight_layout()
    chart_path = output_path / 'db_time_diff_analysis.png'
...
def plot_new_time_diff_chart(new_data, base_time, output_dir, person_info):
    ...
    all_ids = sorted(new_data.keys(), key=lambda x: int(x.replace('未知-', '')) if x.replace('未知-', '').isdigit() else float('inf'))
    # 生成人员列表，确保所有ID都有对应条目
    all_persons = []
    for pid in all_ids:
        if pid in person_info and 'name' in person_info[pid]:
    ...
    # 调整布局并保存
    plt.tight_layout()
    chart_path = output_path / 'new_time_diff_analysis.png'
    plt.savefig(chart_path, dpi=300, bbox_inches='tight')
...
def generate_time_interval_csv(data, output_dir, data_type):
    ...
    # 按ID升序排列（处理ID为数字字符串的情况）
    def sort_key(row):
        # 尝试将ID转为整数排序，如果转换失败则按字符串排序
        try:
            return int(row[0])
    ...
    csv_file = output_path / f'{data_type}_time_interval_analysis.csv'
...
def print_id_statistics(data, sim_data, db_sim_data, new_sim_data, s_db_sim_data, s_new_sim_data):
    ...
    print(f"  各ID对应的记录数量：")
    for pid in sorted(data_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
    ...
    for pid in sorted(sim_data_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
    ...
    for pid in sorted(db_sim_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
    ...
    for pid in sorted(new_sim_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
    ...
    for pid in sorted(s_db_sim_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
    ...
    for pid in sorted(s_new_sim_ids, key=lambda x: int(x) if x.replace('未知-', '').isdigit() else float('inf')):
